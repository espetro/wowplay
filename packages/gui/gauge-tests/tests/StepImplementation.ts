...
export default class StepImplementation {
  ...
  public async clickButton(buttonName: string) {
    if (!this._scenarioFlow) return
    const testId =
      buttonName.toLowerCase() === "browse" ? "folder-browse-btn" : "action-btn"
  ...
  public async verifyButtonVisible(buttonName: string) {
    if (!this._scenarioFlow) return
    this._scenarioFlow.assert("[data-testid='action-btn']", {
      text: buttonName,
      visible: true,
    })
  }
  ...
  public async clickMenuItem(menuItem: string) {
    if (!this._scenarioFlow) return
    const testId =
      menuItem === "Reset Configuration"
        ? "menu-item-reset"
        : "menu-item-toggle-alerts"
    this._scenarioFlow.click(`[data-testid='${testId}']`)
  }
  ...
  public async verifyRunnerEmpty() {
    if (!this._scenarioFlow) return
    this._scenarioFlow.assert("[data-testid='runner-select-trigger']", {
      text: "Select runner...",
    })
  }
  ...
  public async verifyFolderEmpty() {
    if (!this._scenarioFlow) return
    this._scenarioFlow.assert("[data-testid='folder-browse-btn']", {
      visible: true,
    })
  }
  ...
  public async verifyMenuClosed() {
    if (!this._scenarioFlow) return
    this._scenarioFlow.assert("[data-testid='options-menu-btn']", {
      visible: true,
    })
  }
