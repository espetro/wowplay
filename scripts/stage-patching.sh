...
# Production rosettax87 binaries come from the vendored WoWSilicon sources.
# The instrumented fork (vendor/rosettax87_jit) is only used at launch time via
# ROSETTAX87_BIN_DIR — it is NOT staged here; doing so caused an exception
# cascade (12 k+ EXCEPTION_ILLEGAL_INSTRUCTION) in the JIT fork's runtime.
WOWSILICON_RTX87="${REPO_ROOT}/vendor/wowsilicon/Sources/WoWSiliconSwift/Resources/Patching/rosettax87"
...
if [[ ! -f "${WOWSILICON_RTX87}/rosettax87" ]]; then
  echo "error: rosettax87 not found at ${WOWSILICON_RTX87}" >&2
  echo "       ensure the vendor/wowsilicon submodule is present" >&2
  exit 1
fi
if [[ ! -f "${WOWSILICON_RTX87}/libRuntimeRosettax87" ]]; then
  echo "error: libRuntimeRosettax87 not found at ${WOWSILICON_RTX87}" >&2
  exit 1
fi
...
# rosettax87 pair — vendored from WoWSilicon (production-grade, tested binary).
# Staged as "rosettax87" (the name apply_game_patch expects when no ROSETTAX87_BIN_DIR override).
cp "${WOWSILICON_RTX87}/rosettax87"             "${DIST_DIR}/patching/rosettax87/rosettax87"
cp "${WOWSILICON_RTX87}/libRuntimeRosettax87"   "${DIST_DIR}/patching/rosettax87/libRuntimeRosettax87"
